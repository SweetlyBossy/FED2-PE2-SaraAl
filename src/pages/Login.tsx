...
export default function Login() {
  ...
  const handleSubmit = async (e: React.SubmitEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(result.errors?.[0]?.message || 'Login failed. Please check your email and password.');
      }
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-900 via-[#144B9F] to-slate-900 p-4 font-inter">
      ...
      <section className="w-full max-w-md bg-[rgba(177,197,211,0.15)] backdrop-blur-lg border border-white/20 rounded-2xl p-8 shadow-2xl flex flex-col items-center text-center">
        ...
        <div className="mb-8">
           ...
           <p className="text-slate-300 text-sm">Log in to your Nordic Stay account</p>
        </div>
        ...
        <p className="mt-6 text-center text-sm text-slate-300">
          ...
          <Link to="/register" className="text-[#59C095] hover:text-white font-semibold transition-colors">
            Sign up
          </Link>
        </p>
      </section>
