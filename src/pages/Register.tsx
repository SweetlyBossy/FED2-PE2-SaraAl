...
export default function Register() {
  ...
  const validateForm = () => {
    ...
    if (!formData.name.trim()) {
      ...
      newErrors.name = 'Name can only contain letters, numbers, and underscores (_)';
    }
  ...
  const handleSubmit = async (e: React.SubmitEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(responseData.errors?.[0]?.message || 'Registration failed. Please try again.');
      }
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-900 via-[#144B9F] to-slate-900 p-4 font-inter">
      ...
      <div className="w-full max-w-md bg-[rgba(177,197,211,0.15)] backdrop-blur-lg border border-white/20 rounded-2xl p-8 shadow-2xl">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-slate-300 text-sm">Join Nordic Stay to book your next adventure.</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5" noValidate>
          ...
          {/* Name Field */}
          <div>
            <label className="block text-sm font-medium text-slate-200 mb-1" htmlFor="name">Name</label>
            <input 
              id="name"
              name="name"
              type="text"
          ...
          {/* Email Field */}
          <div>
            <label className="block text-sm font-medium text-slate-200 mb-1" htmlFor="email">Stud Email</label>
            <input 
              id="email"
              name="email"
              type="email"
          ...
          <div>
            <label className="block text-sm font-medium text-slate-200 mb-1" htmlFor="confirmPassword">Confirm Password</label>
            <input 
              id="confirmPassword"
        ...
        <p className="mt-6 text-center text-sm text-slate-300">
          ...
          <Link to="/login" className="text-[#59C095] hover:text-white font-semibold transition-colors">
            Log In
          </Link>
        </p>
