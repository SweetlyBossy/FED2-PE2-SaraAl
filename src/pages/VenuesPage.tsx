...
const VenuesPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchVenues = async () => {
      ...
      try {
        setIsLoading(true);
        const response = await fetch(`${baseUrl}/holidaze/venues?sort=created&sortOrder=desc`, {
          headers: {
  ...
  const filteredVenues = useMemo(() => {
    ...
    result = result.filter(venue => {
      ...
      const meetsCountry = filters.country === '' || 
        (venue.location?.country || '').toLowerCase().includes(filters.country.toLowerCase());
      const meetsCity = filters.city === '' || 
        (venue.location?.city || '').toLowerCase().includes(filters.city.toLowerCase());

      return meetsPrice && meetsGuests && meetsRating && meetsWifi && meetsParking && meetsBreakfast && meetsPets && meetsCountry && meetsCity;
    });
      ...
      <div className="relative z-10">
        ...
        <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-12 relative">
            ...
            <div className="flex flex-col gap-3 w-full md:w-auto md:ml-auto z-20 mt-16 md:mt-0">
              ...
              <div className="relative">
                ...
                />
                <svg className="w-4 h-4 text-white absolute left-4 top-1/2 -translate-y-1/2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <div className="relative">
                ...
                {isFilterOpen && (
                  <div className="absolute right-0 top-full mt-2 w-72 md:w-80 bg-deep-navy/95 border border-white/20 rounded-lg p-5 shadow-xl backdrop-blur-md text-white z-50 max-h-[80vh] overflow-y-auto custom-scrollbar">
                    <h3 className="font-bold mb-4 border-b border-white/20 pb-2">Filter Venues</h3>
                    
                    {/* Location Filters */}
                    <div className="mb-4 space-y-3 border-b border-white/20 pb-4">
                      <div>
                        <label className="block text-sm font-semibold mb-1">Country</label>
                        <input type="text" name="country" placeholder="e.g. Norway" value={filters.country} onChange={handleFilterChange} className="w-full bg-white/10 border border-white/30 rounded px-2 py-1 text-sm outline-none focus:border-mint-green" />
                      </div>
                      <div>
                        <label className="block text-sm font-semibold mb-1">City</label>
                        <input type="text" name="city" placeholder="e.g. Tromsø" value={filters.city} onChange={handleFilterChange} className="w-full bg-white/10 border border-white/30 rounded px-2 py-1 text-sm outline-none focus:border-mint-green" />
                      </div>
                    </div>

                    <div className="mb-4">
                      <label className="block text-sm font-semibold mb-1">Max Price: ${filters.maxPrice}</label>
                      <input type="range" name="maxPrice" min="0" max="10000" step="100" value={filters.maxPrice} onChange={handleFilterChange} className="w-full accent-mint-green" />
                    </div>

                    <div className="mb-4">
                      <label className="block text-sm font-semibold mb-1">Min Guests: {filters.maxGuests}</label>
                      <input type="range" name="maxGuests" min="1" max="20" value={filters.maxGuests} onChange={handleFilterChange} className="w-full accent-mint-green" />
                    </div>

                    <div className="mb-4">
                      <label className="block text-sm font-semibold mb-1">Min Rating: {filters.minRating}★</label>
                      <input type="range" name="minRating" min="0" max="5" step="1" value={filters.minRating} onChange={handleFilterChange} className="w-full accent-mint-green" />
                    </div>

                    <div className="grid grid-cols-2 gap-2 text-sm mt-4">
                      <label className="flex items-center gap-2 cursor-pointer">
                        <input type="checkbox" name="wifi" checked={filters.wifi} onChange={handleFilterChange} className="rounded text-mint-green ring-mint-green" /> WiFi
                      </label>
                      <label className="flex items-center gap-2 cursor-pointer">
                        <input type="checkbox" name="parking" checked={filters.parking} onChange={handleFilterChange} className="rounded text-mint-green ring-mint-green" /> Parking
                      </label>
                      <label className="flex items-center gap-2 cursor-pointer">
                        <input type="checkbox" name="breakfast" checked={filters.breakfast} onChange={handleFilterChange} className="rounded text-mint-green ring-mint-green" /> Breakfast
                      </label>
                      <label className="flex items-center gap-2 cursor-pointer">
                        <input type="checkbox" name="pets" checked={filters.pets} onChange={handleFilterChange} className="rounded text-mint-green ring-mint-green" /> Pets
                      </label>
                    </div>
                  </div>
                )}
          ...
          {isLoading ? (
            ...
            <div className="text-center text-red-400 bg-deep-navy/80 p-4 rounded-lg">{error}</div>
          ) : currentVenues.length === 0 ? (
            <div className="text-center text-white text-xl mt-20">No venues found matching your criteria.</div>
          ) : (
            <>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                {currentVenues.map((venue) => (
                  <Link to={`/venue/${venue.id}`} key={venue.id} className="bg-deep-navy/25 border border-white/10 rounded-lg p-4 flex flex-col backdrop-blur-sm hover:bg-deep-navy/35 transition-all duration-300 shadow-lg">
                    <h2 className="text-white font-bold text-lg mb-1 truncate">{venue.name}</h2>
                    <p className="text-white/70 text-xs mb-3 truncate">{venue.location?.city ? `${venue.location.city}, ` : ''}{venue.location?.country || 'Location unavailable'}</p>
                    <div className="w-full h-40 bg-gray-200 rounded mb-4 overflow-hidden">
                      {venue.media && venue.media.length > 0 ? (
                        <img src={venue.media[0].url} alt={venue.media[0].alt || venue.name} className="w-full h-full object-cover hover:scale-105 transition-transform duration-500" onError={(e) => { (e.target as HTMLImageElement).src = '/fallback-image.jpg' }} />
                      ) : (
                        <div className="w-full h-full bg-deep-navy/50 flex items-center justify-center text-white/50 text-sm">No Image</div>
                      )}
                    </div>
                    <div className="mt-auto flex flex-col items-center">
                      <p className="text-mint-green font-semibold text-sm">{venue.rating > 0 ? `${venue.rating} / 5 Rating` : 'Not rated'}</p>
                      <p className="text-mint-green font-semibold text-sm mt-1">${venue.price} <span className="text-xs text-white/70 font-medium">/ night</span></p>
                    </div>
                  </Link>
                ))}
              </div>
              ...
              {totalPages > 1 && (
                <div className="flex justify-center items-center gap-6 mt-12 mb-8">
                  <button onClick={() => setCurrentPage(prev => Math.max(prev - 1, 1))} disabled={currentPage === 1} className="w-10 h-10 rounded-full bg-white/20 text-white font-bold text-xl flex items-center justify-center hover:bg-mint-green disabled:opacity-30 transition-all shadow-lg backdrop-blur-sm">&lt;</button>
                  <span className="text-white font-semibold tracking-wide">Page {currentPage} of {totalPages}</span>
                  <button onClick={() => setCurrentPage(prev => Math.min(prev + 1, totalPages))} disabled={currentPage === totalPages} className="w-10 h-10 rounded-full bg-white/20 text-white font-bold text-xl flex items-center justify-center hover:bg-mint-green disabled:opacity-30 transition-all shadow-lg backdrop-blur-sm">&gt;</button>
                </div>
              )}
            </>
          )}
